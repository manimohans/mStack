...
description: >-
  ...
  workspace, artifact manifest, status report, readiness audit, or next-step recommendation.
---
...
2. Run the recommended specialist skill.
   Use `status`, `audit`, or `next` to see the first incomplete stage and the command to run.

   ```bash
   "$helper" status RELEASE-SLUG
   "$helper" audit RELEASE-SLUG
...
5. Resume from the manifest.
   When work restarts, run `audit` first. Treat a `blocked` verdict as a proof or handoff problem to resolve before drafting or publishing.

   ```bash
   "$helper" audit RELEASE-SLUG --format json
   ```

## Output

Return the session audit verdict or status first. Then state the next mStack command to run and the artifact path that should be produced.
