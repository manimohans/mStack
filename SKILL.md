...
1. **Evidence** - Use the installed evidence-pack skill when proof is scattered across PRs, metrics, screenshots, support notes, or customer quotes.
2. **Context** - Use the installed product-context skill when the raw material is messy or missing.
3. **Angle** - Use the installed angle-review skill when the thesis, positioning, or story is unclear.
4. **Draft** - Use the installed write-product-update skill when it is time to write or rewrite the core update.
5. **Critique** - Use the installed critique-update skill when a draft exists and needs review.
6. **Claim Check** - Use the installed claim-check skill before publication when claims, metrics, quotes, or launch assets need proof review.
7. **Package** - Use the installed launch-pack skill when the story is approved and needs channel assets.
8. **Publish Check** - Use the installed publish-check skill when launch assets need one final readiness verdict before handoff.
9. **Learn** - Use the installed product-retro skill after launch to capture reusable messaging lessons.
10. **Remember** - Use the installed learn skill to search or save durable voice, proof, positioning, customer-language, and launch lessons.
